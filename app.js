...
AoA: ${fa(a.attackAngle)} | Path: ${fd(a.clubPath)}° | Spin Axis: ${fd(a.spinAxis)}° | Misses: ${a.leftMisses}L/${a.straightShots}str/${a.rightMisses}R`;
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, configs.length, " configs \xB7 ", sessions.length, " sessions"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: 8,
      flexWrap: "wrap",
    ...
    onClick: () => {
      ...
      // Try download first, fall back to copy
      try {
        ...
        a.href = url;
        a.download = "golf-fitting-lab-data.json";
        ...
        navigator.clipboard?.writeText(data).then(() => pop("Data copied to clipboard ✓ — paste into a text file and save as .json")).catch(() => {
          // Last resort: show in prompt
          window.prompt("Copy this JSON and save as golf-fitting-lab-data.json:", data);
        });
      }
    },
    style: {
    ...
    onChange: e => {
      ...
      reader.onload = ev => {
        try {
          ...
          if (!d.configs || !d.sessions) throw new Error("Invalid format");
          if (!window.confirm(`Import ${d.configs.length} configs and ${d.sessions.length} sessions? This will replace all current data.`)) return;
          setConfigs(d.configs);
          setSessions(d.sessions);
          storageSave({
            configs: d.configs,
          ...
          pop(`Imported ${d.configs.length} configs + ${d.sessions.length} sessions ✓`);
        } catch (err) {
          pop("Import failed: " + err.message, "err");
        }
      };
      reader.readAsText(file);
      e.target.value = "";
